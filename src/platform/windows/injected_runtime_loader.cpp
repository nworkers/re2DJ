...
#include "injected_runtime_loader.h"
                         ...
                         std::string* error)
{
    if (process == nullptr || primary_thread == nullptr || runtime_base == nullptr ||
        error == nullptr || runtime_path.empty() ||
        SuspendThread(primary_thread) == (std::numeric_limits<DWORD>::max)())
    {
    ...
    HANDLE thread = load_library == nullptr
                        ? nullptr
