...
Windows x86 구현은 `src/platform/windows/direct3d3_com_facade.*`와 전용 `direct3d3_opengl_backend.*`에 있다. launcher의 `--hle-d3d3`는 `DirectDrawCreate` IAT를 facade export로 교체하고 `--hle-display-mode`를 함께 활성화한다. facade는 COM identity/refcount, 가상 HAL 탐색, 16비트 Z/texture format, primary/back/texture surface, device, viewport와 초기 state를 제공한다. RGB565 surface는 CPU-visible DIB backing을 공유하며 GDI bitmap upload, source color key와 color-fill Blt를 처리한다. 공용 `LegacyDrawCommand`는 확인된 XYZRHW/diffuse/specular/texture 좌표를 host API 없이 보존하고, Windows backend는 WGL context와 GLSL shader로 RGB565 triangle strip을 제출하며 Flip에서 swap한다. `DrawPrimitive`와 이어진 texture-stage state null slot은 제거됐고 반복 실행의 다음 경계는 `title.wav` KSND load 제어 종료다. 실제 첫 프레임의 시각 정확성과 나머지 fixed-function state 의미는 아직 검증해야 한다.

직접 x86 `IN`/`OUT`에는 교체 가능한 Win32 import가 없으므로 `include/re2dj/input/legacy_io_port_bus.h`의 플랫폼 중립 raw byte 계약을 `ExecutionBackend` 경계에 둔다. Windows x86 launcher의 `--hle-io-ports`는 1st SE에서 확인된 helper RVA와 opcode, port만 `EXCEPTION_PRIV_INSTRUCTION`으로 처리하고 그 밖의 명령은 guest에 전달한다. 현재 idle state는 active-low `0x101`, `0x102`, `0x106`에 `0xff`, 변화량 계열 `0x103`~`0x105`에 `0x00`이며 물리 버튼 의미는 아직 선언하지 않는다. launcher는 확인된 공유 종료 helper의 EBP frame만 해석해 실제 caller와 format/detail 문자열을 기록한다. profile working-directory mount와 최소 OpenGL draw 경계 뒤 다음 안정 경계는 `title.wav` KSND load 제어 종료다.
...
*The Windows x86 implementation is split between `direct3d3_com_facade.*` and the dedicated `direct3d3_opengl_backend.*`. The facade owns COM lifetime, virtual HAL discovery, 16-bit formats, surfaces, device, viewport, and state. A host-neutral `LegacyDrawCommand` retains the observed XYZRHW, color, and texture coordinates, while the Windows backend submits the RGB565 triangle strip through a WGL context and GLSL shaders and swaps on Flip. The DrawPrimitive and subsequent texture-stage-state null slots are removed; repeated runs next reach a controlled KSND `title.wav` load exit. Visual first-frame accuracy and remaining fixed-function semantics are still unverified.*

*Direct x86 `IN`/`OUT` has no Win32 import to replace, so a platform-neutral raw byte bus under `include/re2dj/input/` forms an execution-backend boundary. Windows option `--hle-io-ports` handles `EXCEPTION_PRIV_INSTRUCTION` only for confirmed 1st SE helper RVAs, opcodes, and ports; everything else remains unhandled. The current idle state uses 0xff for confirmed active-low ports 0x101, 0x102, and 0x106 and zero for unresolved counter-like ports 0x103 through 0x105. The launcher interprets only the confirmed shared helper's EBP frame. After the profile working-directory mount and minimum OpenGL draw boundary, the next stable boundary is a controlled KSND `title.wav` load exit.*
