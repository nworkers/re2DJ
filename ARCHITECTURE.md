...
*Windows x86 COM wiring is separated by DirectX version. The DirectX 6 (1st SE) path resides in `src/platform/windows/direct3d3_com_facade.*`, while the DirectX 7 (4th Trax) path is isolated into `src/platform/windows/directdraw7_com_facade.*`, `src/platform/windows/direct3d7_com_facade.*`, and `src/platform/windows/direct3d7_vertex_buffer_facade.*`, coordinated via `src/platform/windows/directdraw_com_context.h`. The actual renderer remains the shared SDL3/OpenGL backend in `include/re2dj/graphics/sdl3_opengl_backend.h` and `src/graphics/sdl3_opengl_backend.cpp`.*
