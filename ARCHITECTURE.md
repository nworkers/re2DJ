...
Windows x86 첫 구현은 `src/platform/windows/direct3d3_com_facade.*`에 있다. launcher의 `--hle-d3d3`는 `DirectDrawCreate` IAT를 facade export로 교체하고 `--hle-display-mode`를 함께 활성화한다. facade는 COM identity/refcount, 가상 HAL 탐색, 16비트 Z/texture format, primary/back/texture surface, device, viewport와 초기 state를 제공한다. RGB565 surface는 CPU-visible DIB backing을 공유하며 확인된 GDI GetDC/ReleaseDC bitmap upload, source color key와 color-fill Blt를 처리한다. 기존 surface null AV는 제거됐고 다음 경계는 `IDirect3DDevice3::DrawPrimitive`다. OpenGL draw·present는 아직 없다.

직접 x86 `IN`/`OUT`에는 교체 가능한 Win32 import가 없으므로 `include/re2dj/input/legacy_io_port_bus.h`의 플랫폼 중립 raw byte 계약을 `ExecutionBackend` 경계에 둔다. Windows x86 launcher의 `--hle-io-ports`는 1st SE에서 확인된 helper RVA와 opcode, port만 `EXCEPTION_PRIV_INSTRUCTION`으로 처리하고 그 밖의 명령은 guest에 전달한다. 현재 idle state는 active-low `0x101`, `0x102`, `0x106`에 `0xff`, 변화량 계열 `0x103`~`0x105`에 `0x00`이며 물리 버튼 의미는 아직 선언하지 않는다. launcher는 확인된 공유 종료 helper의 EBP frame만 해석해 실제 caller와 format/detail 문자열을 기록한다. profile working-directory mount 뒤 자산 load와 texture surface upload가 진행되며 다음 안정 경계는 `0x0042325c`의 DrawPrimitive null slot execute AV다.
...
*The first Windows x86 implementation lives in `src/platform/windows/direct3d3_com_facade.*`. Launcher option `--hle-d3d3` replaces the `DirectDrawCreate` IAT and also enables logical display mode. The facade provides COM lifetime, virtual HAL discovery, 16-bit formats, primary/back/texture surfaces, device, viewport, and initial state. RGB565 surfaces share CPU-visible DIB backing and handle the confirmed GDI GetDC/ReleaseDC bitmap upload, source color key, and color-fill Blt path. Former surface null AVs are removed; `IDirect3DDevice3::DrawPrimitive` is the next boundary. OpenGL drawing and presentation remain pending.*

*Direct x86 `IN`/`OUT` has no Win32 import to replace, so a platform-neutral raw byte bus under `include/re2dj/input/` forms an execution-backend boundary. Windows option `--hle-io-ports` handles `EXCEPTION_PRIV_INSTRUCTION` only for confirmed 1st SE helper RVAs, opcodes, and ports; everything else remains unhandled. The current idle state uses 0xff for confirmed active-low ports 0x101, 0x102, and 0x106 and zero for unresolved counter-like ports 0x103 through 0x105. The launcher interprets only the confirmed shared helper's EBP frame. After the profile working-directory mount correction, KSND opens `coin0.wav` and `coin1.wav`; the next stable boundary is a null-interface read AV at 0x0042292b.*
