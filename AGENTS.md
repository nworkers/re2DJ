...
이 프로젝트는 에뮬레이터나 가상 머신 전체를 동원하지 않고, EZ2DJ의 **원본 실행 파일을 그대로** 현대 환경에서 실행하는 것을 목표로 한다.
...
This project aims to run the **original EZ2DJ executable as-is** on modern systems without a full emulator or virtual machine.
...
* `ARCHITECTURE.md`
* `docs/WIN32_HLE_PORTING_PLAN.md`
...
7. 원본 바이너리에서 확인하지 않은 내용을 확정된 사실처럼 기술하지 않는다.
...
7. Never state something as confirmed fact unless it was verified against the original binary or a running build.
...
* 원본 바이너리와 HDD 자산에서 확인한 프로젝트 고유 분석은 `docs/analysis/` 아래에 주제별로 둔다.
* PE 형식, Win32 API, DirectX, x86 같은 일반 기술 배경 지식은 `docs/kb/` 아래에 주제별로 두고, 외부 자료에서 얻었다면 출처를 링크한다.
...
* Put project-specific findings from the original binaries and HDD assets under `docs/analysis/`, organized by topic.
* Put generally applicable background knowledge — PE format, Win32 API, DirectX, x86 — under `docs/kb/`, organized by topic and linked to external sources when derived from them.
...
* 작업 중 프로젝트 이해에 중요한 새로운 용어, 표준, API, 실행 형식, CPU 동작이 등장하면 관련 `docs/kb/` 문서를 작성하거나 갱신한다.
* 외부 자료에서 얻은 기술 지식에는 가능한 한 원 사양, Microsoft 공식 문서, CPU 제조사 문서 같은 권위 있는 출처 링크를 가까운 위치에 남긴다.
* `docs/analysis/`는 **확인됨 / 추정 / 미확정**을 반드시 구분해 표기한다.
...
* When important new terminology, standards, APIs, executable formats, or CPU behavior arise, create or update the relevant `docs/kb/` topic.
* Place authoritative external links near derived technical knowledge, preferring original specifications, official Microsoft documentation, and CPU-vendor manuals.
* Always mark statements in `docs/analysis/` as **confirmed, inferred, or unresolved**.
...
* 원본 HDD 내용, 실행 파일, 게임 데이터, 덤프는 **어떤 형태로도 저장소에 커밋하지 않는다.**
* 원본 HDD 내용은 항상 사용자가 지정한 **디렉터리 경로**로 입력받는다. 저장소 안의 고정 경로를 가정하지 않는다.
* 분석 문서에는 바이트 열 전체가 아니라 구조, 오프셋, 관찰된 동작만 기록한다.
* 게스트가 쓰기를 요청한 파일은 원본 디렉터리를 변경하지 않고 별도 overlay 경로에 기록한다.
* 저장소가 원본 자산 없이도 빌드되고 단위 테스트가 통과해야 한다.

## Original Asset Handling Rules

* Never commit original HDD contents, executables, game data, or dumps to the repository in any form.
* Always take the original HDD contents as a user-supplied **directory path**. Never assume a fixed path inside the repository.
* Record structures, offsets, and observed behavior in analysis documents rather than raw byte dumps.
* Write guest file writes into a separate overlay path so the original directory is never modified.
* The repository must build and pass unit tests without any original assets present.
...
* 큰 기능은 하나의 거대 파일에 누적하지 않는다. 플랫폼 공용 상태/정책, 플랫폼 backend, 게스트 ABI 연결처럼 책임이 다른 부분을 별도 header/source로 분리한다.
* 기존 파일에 기능을 추가할 때 독립적으로 이름 붙일 수 있는 하위 시스템이면 전용 파일로 추출하고, 통합 지점에는 orchestration과 adapter 코드만 남긴다.
* 호스트 OS API를 공용 코어에서 직접 호출하지 않는다. 공용 코어는 `std::filesystem`과 플랫폼 인터페이스만 사용한다.
* 프로젝트 기본 라이선스는 `BSD 3-Clause License`를 기준으로 한다.
* 서드파티 오픈소스 도입 시 GPL, LGPL, AGPL 같은 전염성 라이선스는 사용하지 않는다. Wine(LGPL)은 이 규칙에 따라 코드 수준에서 사용하지 않으며, 공개 문서만 참고 자료로 인용한다.
...
* Never call host OS APIs directly from the platform-neutral core. The core uses `std::filesystem` and platform interfaces only.
* Use the `BSD 3-Clause License` as the default project license baseline.
* Do not introduce third-party open source with copyleft licenses such as GPL, LGPL, or AGPL. Wine (LGPL) is therefore excluded at the code level; cite only its public documentation as reference material.
...
이 저장소는 원본 게임 바이너리나 데이터를 포함하지 않으며 배포하지도 않는다. 사용자는 합법적으로 보유한 자산에 대해서만 이 런타임을 사용한다.

## Legal Scope

This repository neither contains nor distributes original game binaries or data. Use this runtime only with assets you legally own.
